...
def nn_nnn_generate(Lx,Ly,Jnn,Jnnn,p,seed):
	...
	neighbors = np.zeros((8,Lx*Ly),dtype=int) ### This is the list of 4 nn indices of each site stored in the order [+x,+y,-x,-y]
	...
	for r in sites:
		...
		neighbors[:,r] = np.array([rpx,rpy,rmx,rmy,rpxpy,rmxpy,rmxmy,rpxmy],dtype=int)
		

	return 0.5*( J_matrix + np.transpose(J_matrix)), neighbors
...
### Implements annealing dynamics but memory efficient -- only saves a number of observables rather than the whole spin state 
def anneal_dynamics_low_mem_nnn(J_matrix,neighbors,nsweeps,temperature_schedule,distances):
	if not isinstance(temperature_schedule,np.ndarray): temperature_schedule = np.array([temperature_schedule]) ### Recast single float as an array of length 1 for technical
	...
	def MCstep(spins):
		...
		curie_field = sum( J_matrix[i,r]*spins[i] for i in neighbors[:,r] )
		...
		### Glauber dynamics has probability of flip e^(-beta dE)/(1+ e^(-beta dE))
		pr = rng.uniform()
		if pr < np.exp(-betadE)/(1.+np.exp(-betadE)):
			tmp[r] *= -1 
			betadE_return = betadE
...
def anneal_dynamics_lattice(lattice,nsweeps,temperature_schedule,distances):
	if not isinstance(temperature_schedule,np.ndarray): temperature_schedule = np.array([temperature_schedule]) ### Recast single float as an array of length 1 for technical
	...
	def MCstep(spins):
		...
		curie_field = sum( latt.J_matrix[i,r]*spins[i] for i in nns )
	...
	for n in range(nTs):
		...
		### Just first time step we compute the observables for the entire system
		### !!! ASSUMES NN J MATRIX STILL 
		energies[n,0] = calc_energy(spins,latt.J_matrix)
		mags[n,0] = np.mean(spins) 
